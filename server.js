...
// ---------- Events + roster ----------
// By default only "published" events are returned - draft events created by
// the schedule builder are invisible to everyone (including admins on the
// normal Roster page) until they're published. Pass ?status=draft (admins
// only) to fetch just the drafts for review, or ?status=all to fetch both.
function isoDate(d) {
...
app.get('/api/events', requireAuth, (req, res) => {
  ...
  const params = [];
  if (upcomingOnly) { query += ' AND event_date >= ?'; params.push(today); }
  if (status === 'draft') query += ' AND published = 0';
  else if (status !== 'all') query += ' AND published = 1';
...
// Edit an event's title/time/notes (e.g. filling in the specific topic for a
// generated placeholder like "Wednesday Bible Class" before publishing it).
app.put('/api/events/:id', requireAuth, requireAdmin, (req, res) => {
  const existing = db.prepare('SELECT * FROM events WHERE id = ?').get(req.params.id);
  if (!existing) return res.status(404).json({ error: 'Not found' });
  const title = req.body.title !== undefined ? String(req.body.title).trim() || existing.title : existing.title;
  const notes = req.body.notes !== undefined ? (String(req.body.notes).trim() || null) : existing.notes;
  const event_time = req.body.event_time !== undefined ? (req.body.event_time || null) : existing.event_time;
  const event_date = req.body.event_date || existing.event_date;
  db.prepare('UPDATE events SET title = ?, notes = ?, event_time = ?, event_date = ? WHERE id = ?')
    .run(title, notes, event_time, event_date, req.params.id);
  res.json({ ok: true });
});
...
// ---------- Admin: schedule builder (bulk-generate a draft, review, publish) ----------
// Creates the recurring weekly meetings for a date range as DRAFT events
// (published = 0) so an admin can review/edit/auto-fill them privately before
// anyone else sees the new period. Skips a date+title combo that already
// exists (safe to run more than once over the same range).
const SCHEDULE_MEETING_TIMES = {
  'Sunday Memorial Meeting': '11:00',
  'Sunday Evening Lecture': '18:00',
  'Wednesday Bible Class': '20:00'
};

app.post('/api/admin/generate-schedule', requireAuth, requireAdmin, (req, res) => {
  const { start_date, end_date } = req.body || {};
  if (!start_date || !end_date) return res.status(400).json({ error: 'Start and end date required' });
  if (end_date < start_date) return res.status(400).json({ error: 'End date must be on or after the start date' });

  const existing = db.prepare('SELECT event_date, title FROM events WHERE event_date >= ? AND event_date <= ?').all(start_date, end_date);
  const existingSet = new Set(existing.map(e => `${e.event_date}|${e.title}`));

  const insertEvent = db.prepare('INSERT INTO events (event_date, event_time, title, notes, published) VALUES (?, ?, ?, NULL, 0)');
  const insertAssignment = db.prepare('INSERT INTO assignments (event_id, role, user_id, assigned_name) VALUES (?, ?, NULL, NULL)');
  ...
  while (cur <= end) {
    ...
    const dow = cur.getDay(); // 0 = Sunday, 3 = Wednesday
    const titles = dow === 0 ? ['Sunday Memorial Meeting', 'Sunday Evening Lecture']
      : dow === 3 ? ['Wednesday Bible Class']
      : [];
    titles.forEach(title => {
      const key = `${iso}|${title}`;
      if (existingSet.has(key)) { skipped++; return; }
      const result = insertEvent.run(iso, SCHEDULE_MEETING_TIMES[title], title);
      const eventId = result.lastInsertRowid;
      (ROLE_PRESETS[title] || []).forEach(role => insertAssignment.run(eventId, role));
      existingSet.add(key);
      created++;
    });
...
app.post('/api/admin/publish-schedule', requireAuth, requireAdmin, (req, res) => {
  const { start_date, end_date } = req.body || {};
  let query = 'UPDATE events SET published = 1 WHERE published = 0';
  const params = [];
...
app.post('/api/admin/discard-draft', requireAuth, requireAdmin, (req, res) => {
  const { start_date, end_date } = req.body || {};
  let query = 'DELETE FROM events WHERE published = 0';
  const params = [];
  if (start_date) { query += ' AND event_date >= ?'; params.push(start_date); }
  if (end_date) { query += ' AND event_date <= ?'; params.push(end_date); }
  const result = db.prepare(query).run(...params);
