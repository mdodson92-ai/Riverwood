...
CREATE TABLE IF NOT EXISTS events (
  ...
  notes TEXT,
  published INTEGER NOT NULL DEFAULT 1, -- 0 = draft (only admins see it), 1 = visible to everyone
  created_at TEXT DEFAULT (datetime('now'))
);
...
// Ensure published exists for databases created before this column was added.
try {
  db.prepare('SELECT published FROM events LIMIT 1').get();
} catch (e) {
  db.exec('ALTER TABLE events ADD COLUMN published INTEGER NOT NULL DEFAULT 1');
}
